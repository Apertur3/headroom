...
export function parseUsageImportOptions(argv: readonly string[]): UsageImportOptions {
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    if (arg.startsWith("--")) {
      const flag = arg;
      if (flag === "--source" || flag === "--principal" || flag === "--path" || flag === "--job" || flag === "--max-bytes" || flag === "--format") {
        ...
        if (flag === "--source") {
          ...
          if (formatStr !== undefined) throw new Error("Duplicate flag: --format");
          if (val !== "auto" && val !== "claude" && val !== "codex") throw new Error("Invalid value for --format");
          formatStr = val;
        }
      } else if (flag === "--json") {
        if (json) throw new Error("Duplicate flag: --json");
