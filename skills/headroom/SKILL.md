...
- `headroom can <action-class> [--allow-unknown]` : go / no-go for an action class.
...
Take a lease before fanning out work: `headroom lease start --owner <name> --meter <meter_id> --expect <percent>`. Pass `--owner <name>` to `headroom can` so your own reservation is not counted twice, and end the lease when the work is done. Other orchestrators on this machine see active leases.
